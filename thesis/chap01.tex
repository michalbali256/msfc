...
\todo{Anglicky vety maj pevny poradi slov, hlavni je ze subject verb object. Tady to mas presne obracene, ma byt: Traffic scheduling is one of the pieces ...}
One of the pieces of puzzle that make the Internet and generally packet-switched networks work is traffic scheduling. \XX{It lies between the second and the~third layer of OSI (or the TCP/IP). That means its turn comes after the network layer does its job: it determined the packet's next hop and particularly the link, through which the device is going to transmit the packet. Before the device hands it over to the data link layer and actually sends it, it waits in a buffer. The buffer is needed because of asynchronism of packet-switched networks (further reasons why buffers are needed are discussed in \autoref{chap:bb}).}\todo{traffic scheduling neni na zadny vrstve, jen chyta pakety co v routerech bezne prochazej mezi vrstvama, bezne network->link (v kernelu) a link->phy (na sitovy karte). Muzes s cistym svedomim rict ze buffering je potreba due to asynchronous nature of the data transfer in packet-switched networks, ale moc bufferingu taky neni dobre (tradeoff: moc malej buffer udela moc velkej loss, moc velkej buffer udela moc velkou latenci). Nejprimitivnejsi pouzitelny buffery jsou FIFO (FCFS), traffic scheduling je z pohledu tyhle prace o tom ze misto FIFO pouzijes jinej chytrejsi, adaptivnejsi nebo jinak dokonalejsi typ fronty kterej navic s tema paketama dela nejakou magii aby se zlepsila nejaka vlastnost site.}
...
\begin{itemize}
	...
	\item Packet loss --- ratio of packets\XX{,}\todo{carka pred vedlejsi vetou (hlavne uvozenou that) se vetsinou nepise, pokud to neni potreba kvuli jednoznacnosti} that never reach their destination to all packets.
...
\todo{tohle asi bude zmineny uz vejs, navic to problems arise a musime bejt vic sofistikovany je dost melky tvrzeni :D}
...
There is wide range of requirements that the ideal traffic scheduler should satisfy. It starts with avoiding poor performance \todo{of what?}, and other negative phenomena observed in networks, continues with enforcing fairness for all applications (users) \todo{starts, continues,...and reaches something?}. Internet service providers use scheduling to define the shape of traffic provided to customers. Researchers have realised the potential \todo{which potential?} years ago and this chapter offers a summary of basic concepts used in the area.
...
% >>> packet-switched networks jsou z definice connectionless, connection-oriented networks jsou prakticky mrtvy.


\section{Bufferbloat}
\label{chap:bb}

In the gateways \todo{mozna by to chtelo drobnou definici toho co je gateway} of packet-switched networks, short-term differences between arrival and departure rate \todo{rate of what?} naturally happen \todo{occur.}. To balance these bursts\todo{co je burst? chces rict `the bursts of incoming traffic'?}, gateways use buffers --- packets \todo{which packets? these that cannot be transmitted immediately?} \todo{are put to} wait in queues until they can be sent towards their destination \todo{until the sending interface finishes transmitting of previous packets?}. This \todo{also} helps avoiding outgoing \todo{hint: vzdycky kdyz za sebe pises 2x slovo s -ing na konci tak to skoro urcite neni anglicky. Tady asi `helps to avoid the'} link starvation and increases throughput. Without buffers, the gateway has no place to store incoming packets and they get dropped, which decrease throughput. \todo{ty posledni 2 vety chces asi prohodit aby to na sebe trochu navazovalo.}


However, the more packets are in a queue, the longer packets stay in it and the longer it takes \todo{for them} to be delivered. Unfortunately, queues in modern networks tend to fill up and stay 'bloated' \cite{Gettys:2012:BDB:2063176.2063196} --- some amount of packets always stays in the queue, even if there are no incoming bursts to balance. \todo{trochu jsem to prevoral}
...
To understand why \todo{the} queues become bloated, let\XX{'s} \todo{do odbornyho textu zadny apostrofy, maximalne jako privlastneni k vlastnim jmenum} take a look at Transport Control Protocol (TCP), which is \todo{probably/currently} the most widely used layer 4 protocol. It achieves its reliability \todo{reliability of what? of packet delivery. `its' netreba.} with the following policy: After a packet is transmitted, the receiver sends an ACK packet back to sender \todo{takze kdyz sender transmituje paket, tak to receiver vzdalene magicky vytusi a hned mu posle ACK?}, to let him know which packets were successfully received, and which packets \XX{must the sender} resend. \XX{But} waiting on ACK after each packet would provide miserable throughput and \todo{since? because?} most of the time both sides would wait for \todo{a} single packet. Instead, the sender goes ahead and transmi\XX{t} packets without getting ACKs on previous ones. This fills the whole route, so at all times one packet is being sent, and one packet is being received. The ACKs that come back retain the same spacing. The situation is displayed in Figure \ref{fig01:no_bottle}. \todo{mozna chces tu referenci na obrazek driv, aby bylo pochopitelny co znamena to `fills the route'}

Figure \ref{fig01:no_bottle} shows connection with constant bandwidth along whole path, which is \todo{,unfortunately,} rarely the case in today\XX{'s} Internet. Typical path between the communicants consists of many hops with links of different bandwidth. \XX{That means somewhere must be a bottleneck - a link with lowest bandwidth.} \todo{One of the links will inevitably become a bottleneck?} In the resulting situation, if the sender would transmit \todo{2nd conditional se sem moc nehodil+pridal sem to in the situation} at the rate of the adjacent link \todo{which one? his.}, the network would become congested. The bottleneck link would not be able to forward all the incoming traffic, its buffer would fill, and the rest of the arriving packets would be dropped. That would actually induce even more traffic, because the sender would try to resend the dropped packets.


%tcp by potrebovalo vediet aky badwidth ma bottleneck
\todo{To avoid this problem,} TCP must \todo{nemusi, ale was designed to} limit the upstream rate. In ideal case, it would work \todo{work? transmit?} at the rate of the slowest link. However, the sender does not have any information about bottleneck \XX{whatsoever}. Thus \todo{thus znamena neco jinyho. Mozna chces rict `To approximate this rate'}, TCP uses \todo{a?} congestion window \todo{algorithm? approach?}: it is \todo{what?} amount of bytes TCP can send without receiving an ACK. When an ACK is received, it frees space in congestion window, and TCP may permit sending the next packet, filling the window again. \todo{what if the ACK is not received?} The sender receives the first ACK after round trip time (RTT) --- the time it takes to transmit a packet through path sender-receiver-sender. \todo{tu posledni vetu je potreba obratit. The time it takes .... is called RTT; this time can be also approximated from the time between sending the data and rcving ACK.}

To maximize throughput, size of congestion window should be at least the (bottleneck) bandwidth-delay product (product of bandwidth and RTT) bytes. \todo{tohle muzes vysazet jako vzorecek} In this \todo{that?} case, the sender fills the pipe with packets just before it receives the first ACK and is allowed to send the following packets. On the other hand, if the congestion window exceeds the bandwidth-delay product, the excessive packets stay in queues along the path and cause delay. 
...
Bufferbloat is caused by \todo{a} mismatch between \todo{the size of the?} congestion window and actual RTT \cite{CoDel}. In reality, estimating the window size is difficult: Always-changing network load affec\XX{t} both RTT and bandwidth, \todo{and?} paths change thanks to \todo{due to? because of possible?} rerout\XX{e}ing. Buffers can really only be measured at the bottleneck and even there it is hard to differentiate between useful and useless buffers that only cause delay. \todo{posledni vetu nechapu.}
...
Figure \ref{fig02:bottle_1} shows a starting TCP communication. The illustrated network consists of 3 subnetworks. The left and the right have bandwidth 30 Mb/s, the middle one has 10 Mb/s. One packet has 30kb \todo{to je poradnej paket, 3750 bajtu}. The TCP Congestion window is \todo{set to?} 20 packets \todo{nebylo to mereny v bytech?}. The sender starts by transmitting whole window of packets \XX{back to back}. \XX{They} arrive at edge of the bottleneck network and get enqueued there, because of the bandwidth difference between left and middle subnetworks. \XX{A packet arrives from the left network every 1 ms, but only every 3 ms is one dequeued.} At the right side \todo{After continuing to the right subnetwork?}, \XX{they} \todo{the packets? the networks?} retain spacing given by \todo{the} bottleneck, as shown in Figure \ref{fig03:bottle_2}. The receiver turns incoming packets into ACKs with the same spacing. The sender then sends one packet of data for each ACK it gets.

This way, after one RTT, the whole connection gets into state of equilibrium. The bottleneck is fully utilized, so the throughput is as high as possible. \XX{However, the queue at the middle network never gets empty. At first, it has to hold the burst of the whole congestion window. Once all 20 packets are sent, 14 of them are waiting in the queue, 6 are already forwarded towards the middle network. Then, no packets are enqueued, and every 3 ms one is dequeued. Until one RTT passes, first ACK is delivered, and next packet is sent into left network. After that, one packet arrives and one leaves every 3 ms. 5 packets stay there and every one of them waits long 15 milliseconds. Also, they block buffer space other connections could use.} \todo{tohle je chaos, mozna proste jen popis to ekvilibrium a co je na nem spatne.}
...
The reason, why the packets stay in the buffer in Figure \ref{fig03:bottle_2} is\XX{,} that the congestion window is set to 5 more packets than the bandwidth-delay product. Determining the size of the congestion window is not an easy task. At start of TCP \todo{TCP communication?}, slow start algorithm \cite{Jacobson:1988:CAC:52324.52356} is \todo{is always used? may be used?} used to determine the size of congestion window. It grows exponentially \todo{the algorithm grows exponentially? or sets the C.W. to grow exponentially?}, increasing size of the window with each ACK received, until a threshold is reached or a packet is dropped. After that, congestion avoidance takes over \todo{continues?} to maintain it. Over the years \XX{of TCP in service}\todo{to zni jako ze TCP slo do valky}, many variants \todo{of TCP? of congestion avoidance algorithms? of slow start algorithms?} were introduced. Currently, CUBIC \cite{CUBIC} is used widely. 

%emhasize that it is a way to communicate
The congestion windows are managed by the TCP endpoints, while congestion and bufferbloat takes place in gateways along the path. \XX{There is no direct communication channel between them. When the buffer becomes full, the next packets must be dropped simply because there is no room for them. When TCP does not receive an ACK, it the congestion avoidance algorithm reduces the window and slows down.} \todo{mozna chces rict ze by asi bylo logicky kdyby si to ty strany navzajem povedely, akorat ze na to zatim neni moc support. To ECN bych tam dal jen jako priklad.}
...
\XX{This all indicated, that simple tail drop queues, which drop packets only when they are full may be superseded by more sophisticated queues right in the gateways.} \todo{A common solution to the bufferbloat problem involves?} It involves the counter-intuitive idea of dropping \todo{or ECN-marking} perfectly good packets, even if buffer is not full yet. \XX{But }the drop indicates a problem \todo{to whom? to the endpoints that may start to optimize the upstream rate?}, while still having \todo{some} room for balancing bursts. This approach is called Active Queue Management (AQM), and is recommended to use throughout the Internet \todo{by whom?}.
