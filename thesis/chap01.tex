...
\begin{algorithm}[p]
	...
	\label{alg:CoDel}
	\begin{algorithmic}
		\Function{controlLaw}{\textcolor{blue}{Time} t, \textcolor{blue}{Integer} count}
		\State \Return t + \textsc{Interval} / sqrt(count)
		\EndFunction
		\Function{enqueue}{\textcolor{blue}{Packet} pkt}
		\State pkt.timestamp $\leftarrow$ clock()
		\State push(queue, p)
		\EndFunction
		\Function{dequeue}{}
		\State \textcolor{blue}{Time} now $\leftarrow$ clock()
		\State \textcolor{blue}{DoDequeueResult} r $\leftarrow$ doDequeue(now)
		
		\If {dropping} \Comment CoDel is in dropping state
		\If{\textbf{not} r.okToDrop} \Comment sojourn time below \X{Target}
		...
		\State drop(r.pkt)
		\State count += 1
		\State r $\leftarrow$ doDequeue(now)
		\\
		\If{\textbf{not} r.okToDrop}
		\State dropping $\leftarrow$ false
		\Else
		...
		\ElsIf{r.OkToDrop}
		\State \Comment {\parbox[t]{.8\linewidth}{ OkToDrop returned true (sojourn time has been above \X{Target} for \X{Interval}), so enter dropping state}}
		\State drop(r.pkt)
		\\
		\State r $\leftarrow$ doDequeue(now)
