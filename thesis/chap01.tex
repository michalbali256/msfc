...
One of the pieces of puzzle that make the Internet and generally packet-switched networks work is traffic scheduling. It lies between the second and the~third layer of OSI (or the TCP/IP). That means its turn comes \todo{its turn in what? guess: in each single router} after the network layer does \XX{did?} its job: it \XX{determined the packet's next hop and particularly the link,} through which the device is going to transmit the packet. Before the device hands it over to the data link layer and actually sends it, it waits in a buffer. The buffer is needed because of asynchronism of packet-switched networks (further reasons why buffers are needed are discussed in \autoref{chap:bb}).

As the Internet grows, more and more different traffic flows \todo{tady to flows vypada dost jako podstatny jmeno, jako "traffic flows do something"...mozna mirne prefrazovat} through and more pressure is put on maintaining acceptable quality of service \XX{QoS}. Ever-rising number of services are now provided \todo{spis chces asi rict `occupy' nebo tak neco} through internet protocol, that require \XX{certain} standards: IPTV, VoIP, teleconferencing. % The Internet consist of countless protocols, principles, ideas and algorithms. None of them are perfect, and perfecting any piece of puzzle may have far-reaching consequences.
...
The term quality of service is used to denote overall performance of the Internet service. Various measurements may be required for different services. Voice over Internet Protocol (VoIP), which enables us\todo{allows users to} to phone each other over the IP \XX{protocol} requires small delay\todo{of what?}, packet loss and jitter. Internet protocol television (IPTV) additionally requires sufficient throughput. On the other hand, e-mail does have little requirements whatsoever. Here are some examples of measurements that affect quality of service\todo{tadyto by bylo dobry rict jako `tyhle kvality se bezne oznacujou za QoS a my je budeme pozorovat', at to ma nejakou definitorickou hodnotu. Vklidu do tabulky. V ty samy tabulce muzes udelat i nejakej `feature matrix' o tom jaky servicy potrebujou jaky vlastnosti}:
\begin{itemize}
	...
	\item Packe\XX{r} errors --- packet error happens, when the delivered packet is different than the sent packet.
	\item Delay --- time that elapses between packet sending and receiving.
	\item Jitter --- variance in delay of consecutive packets.
\end{itemize}

The natural simplest solution \todo{for what} is to implement first-come first-serve (FCFS) queue\todo{dost lidi to zna jako FIFO, mozna by nebylo spatny to napsat i tady}. However, as shown throughout this chapter, problems arise\todo{from what and when?}, that can\todo{must} be solved by using more sophisticated methods instead.

There is wide range of requirements ideal traffic scheduler should satisfy\todo{chce to predchozi pevnou definici traffic scheduleru}. \XX{It starts with}\todo{These include?} avoiding poor performance, and other negative phenomena observed in networks, continues with enforcing fairness for all applications (users). Internet service providers use scheduling to define the shape of traffic provided to customers. Researchers have realised the potential years ago and this chapter offers a summary of basic concepts used in the area.

\todo{veta na zaver o carkach: Pred vedlejsima vetama ktery jde uvodit pomoci `that' se carka nedela, jen v extremnich pripadech kdy tam je fakt potreba pauza v reci. Pred and se carka nedela, krome Oxford Comma ktery slouzi pro odstraneni nejednoznacnosti. Obecne pokud je veta jednoznacne pochopitelna bez carky, carku nedelej.}
...
In gateways of packet-switched networks, short-term differences between arrival and departure rate naturally happen\todo{mozna spis occurr naturally, as a result of XXX}. To balance these bursts, gateways use buffers --- packets \todo{which packets} wait in queues \todo{queues are buffers? where are the queues?} until they can be sent towards their destination. This helps avoiding outgoing link starvation \todo{mozna definice starvation} and \todo{therefore} increases throughput. Without buffers, the gateway has no place to store incoming packets and they get dropped\todo{in what situation}, which decreas\XX{e} throughput.


However\todo{tady jeste nemas nic proti cemu bys daval However --- mozna chces rict ze by asi ted mohlo vypadat, ze vetsi fronta je lepsi, jenze ne. :D}, the more packets are in a queue, the longer packets stay in it and the longer it takes \todo{for them} to be delivered\todo{forwarded?}. Unfortunately, queues in modern networks tend to fill up and stay 'bloated' \cite{Gettys:2012:BDB:2063176.2063196} --- some amount of packets always sta\XX{y} in the queue (it never becomes empty), even if there are no incoming bursts to balance. \todo{Proc to je spatne? (naprosto zbytecne se zveda ping)}
...
To understand why queues become bloated, let's take a look at Transport \todo{Transmission!} Control Protocol (TCP), which is \todo{currently} the most widely used layer 4 protocol. It achieves its reliability with the following policy: After a packet is transmitted, the receiver sends an ACK packet back to sender, to let him know which packets were successfully received, and which packets must the sender resend. \XX{But} waiting \XX{on} ACK after each packet would provide miserable throughput and most of the time both sides would wait for single packet. Instead, the sender goes ahead and transmi\XX{t} packets without getting ACKs on previous ones. This fills the whole route, so at all times one packet is being sent, and one packet is being received. The ACKs that come back retain the same spacing. The situation is displayed in Figure \ref{fig01:no_bottle}.

\todo{tadyto by chtelo trochu preskupit --- bufferbloat je obecne ten stav, ze buffery jsou zbytecne plny. To s tim TCP je jeden priklad toho, ze ani kdyz se endpointy dost snazej s tim neco udelat, tak maj solidni sanci to zmrsit. Also, chce to referenci na TCP RFC.}
...
\todo{tady uz trochu netusim jak to souvisi s predchozim odstavcem....mozna by to chtelo podsekci a nejakej mini uvod?}
...
It \todo{what? aha codel.} has 2 parameters:
\begin{itemize}
...
--- je tu treba nejaky takyto transition? --- \XX{je fajn}
