...
\todo{cleny}
\label{chap3}
As discussed in previous chapter, we implemented MSFC in ns-3 to see how well it performs in simulated conditions and compare it to other traffic schedulers. \todo{for the benchmark} We simulate \todo{a} part of Wi-Fi network \todo{infrastructure} an ISP may manage \todo{mozna: similar to networks of wireless-based Internet providers}, \XX{since our scheduler aims to ordinary routers at the "last mile" of the Internet --- a few hops near the customer}\todo{which is the environment targeted by the scheduler?}. We simulate several types of \XX{traffic common users generate} and evaluate \todo{the} performance of \XX{commonly}\todo{rovnou je listni} used traffic schedulers. \todo{mozna rovnou napis ze ty schedulery davas na vsechny interfacy vsech ISP-owned zarizeni po ceste}
...
The simulated network is illustrated in Figure \ref{fig11:sim_layout}. The network is tree-shaped. The topmost node is server, it represents \todo{that serves as} the rest of the Internet. Further, there is part of the infrastructure of an ISP. There are gateway nodes G1-G7. All gateways have 2--5 children. The leafs of this tree are access points (APs) of wireless networks. Finally, 8--12 clients (customers of ISP) are connected to each AP, \XX{with the default seed} this results in total of 140 clients \todo{takze fakt simulujes 140 klientu nebo mas jinej seed? staci to jednodusejc bez kecani o tom seedu.}.

Each client is connected to exactly one AP. The clients are connected with 802.11ac Wi-Fi --- they share the Wi-Fi bandwidth. APs and gateways are connected with point-to-point links with 100Mbps bandwidth and 5ms delay. The server is connected to the root node G1 of the tree with\todo{using, at tam neni 2x with} 1000Mbps link with 50ms delay.

The clients download and upload data. All the traffic flows between the server and one of the clients\todo{vsechen traffic jde na jednoho klienta?}. There are several types of applications \todo{tj. connections/flows?} that model various behaviour \todo{behaviorS?} of real-life \todo{real-world Internet users} clients. The types are listed in table \ref{tab:traffic}. They vary in transport protocol used, \todo{average?} size of a single packet and the data rate at which they generate traffic. The data rate may be constant --- in that case the application generates packets on regular basis (e.g. it sends a packet every 5 milliseconds), or it may be variable.

The applications with variable bit rate turn on and off on irregular basis. During off period \todo{when switched off}, it sends zero packets \todo{does not send any packets...} and during on time \todo{switched on} it sends packets at configured constant rate. The on times and off times \todo{durations?} are generated randomly --- using normal distribution $\mathcal{N}(1,1)$. \todo{z tohohle normalniho rozdeleni muze vypadnout zaporna hodnota, jak se to projevi na tom case?}
...
\todo{In the benchmark} \XX{To evaluate MSFC}, we ran multiple simulations with different schedulers. Each time, we installed the evaluated scheduler to all nodes (NetDevices) of the simulation. We tested PfifoFast, CoDel, FQ CoDel and MSFC. We have used all schedulers with default parameters \todo{asi radsi pasiv, All schedulers were..., a jeste ze ty parametry jsou defaultni pro ns-3 a ne pro neco jinyho, navic defaultni parametry pro MSFC nejdou nikde jednoduse dohledat, nebylo by spatny je tady vylozene vysypat do tabulky nebo se aspon odkazat na nejaky misto kde je mas napsany v textu.}.

We measured throughput, packet loss, delay and jitter using ns-3 module FlowMonitor \cite{flowMonitor}. Throughput is the rate at which packets flow through a node measured in bytes per second. Delay is the time taken to transmit a packet from sender to receiver. Jitter is \todo{the} variation of delay. FlowMonitor computes \todo{approximates the} jitter of a packet \XX{simply, only relatively to the previous packet} \todo{using only ...}:
\[
	\textsc{Jitter}(P_N) = \abs{\textsc{Delay}(P_N) - \textsc{Delay}(P_{N-1})},
\]
where $P_N$ is the n-th received packet. \XX{We measure all the values in the IP layer. That means we count in IP header, but not frame header (link layer header). Also, TCP-retransmitted packets count separately.}\todo{to bych uplne vynechal pokud to neni pro neco dulezity pozdejc}
...
\XX{Also,} we only present results of `download' \todo{tady sem upravil apostrof} direction --- the upload direction does not have enough throughput, and since we use full-duplex \todo{tady sem upravil pomlcku z -- na -} point-to-point links, the results are not interesting. \XX{The traffic schedulers do not have effect, if the network has enough bandwidth to transmit all potential traffic.}



\section{Simulation A}
\todo{prejmenovat na simulation results, simulace B bude specialni priklad}
...
\XX{We will shed some light on all the numbers. }\todo{As seen in the table} The throughput of the \emph{torrent} flows in one--queue schedulers CoDel and pfifo\_fast is dramatically higher than FQ CoDel and MSFC. That naturally results in all other flows receiving less throughput, since all schedulers manage to utilize the links similarly (\todo{see} throughput row in Table \ref{tab:results_A}). The reason is\XX{,} that UDP does not have any congestion control and thus sends as much packets as possible regardless of being dropped. CoDel and pfifo\_fast mix all the packets in one queue so the TCP flows notice the congestion and slow down. The result is\XX{,} that misbehaving users are actually favoured\todo{favoured neni asi to spravny slovo, spis advantaged by the setup}. FQ CoDel and MSFC employ fair queueing principles (see \autoref{sec:fair_queueing}) that isolate the misbehaving users and thus reduc\XX{ing} the throughput of misbehaving flows. \todo{uncongested flow nemusi bejt nutne misbehaving...mozna by to slo tam nekde vrazit aby bylo jasny co se tim misbehaving mysli presne}
...
The measured jitter is close to zero \todo{where? in all flows?}. The average values of 0--2 from Table \ref{tab:results_A} \XX{represent the jitter well} and jitter of all types of flows was negligible --- always less than 6\% of delay. \XX{The simulation testbed does not offer network big enough to observe any significant jitter.} \todo{tohle bych uplne vynechal protoze prvni co oponenta napadne je ze mas moc malou sit :D }

\XX{Another thing we can observe is,} \todo{tady by to mozna chtelo zduraznit ze tohle je ten hlavni vysledek :D} that MSFC responds to the assigned priorities well. \emph{SSH}, \emph{VoIP} and \emph{game} flows achieved almost the same quality of service. Not surprisingly, since we assigned the highest priority to \emph{TV} flows, average \emph{TV} flow received 3185 kbps with MSFC, which is much more than 1327 kbps it got from FQ CoDel. Of course, the rest of flows with lesser priorities had less throughput, but the decrease again scaled with the priorities.

Figure \ref{fig:delay_flows_A} shows detailed comparison of delays of FQ CoDel and MSFC (we do not compare CoDel and pfifo\_fast, since their delay was much higher). \todo{muzes tam sypnout ze ten MSFC delay je trochu horsi, ale o tech par milisekund to nevadi}
...
\XX{First of all,} the \todo{results of?} simulation A \XX{supported previous fair queueing research} \todo{confirms the expectations?}. Although UDP with no congestion control whatsoever is extreme example of flow misbehaviour, it confirmed that FQ (SFQ) principles are desired in traffic scheduling. \todo{tahle veta nedava smysl}
